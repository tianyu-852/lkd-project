...
<!-- 		<view class="safe-content">1.为及时了解及监督服务质量、解决服务纠纷平台有权对通话进行录音及查询</view>
		...
		<view class="safe-content">4.订单规则<br>如合同审查、合同起草、文书起草等后期需修改的服务，将在交付7天后自动结束该订单</view>
		<view class="safe-content">5.如您还有其他任何疑问，可以联系律师管家或者拔打热线0577-86859588，我们会有专业客服为您服务</view>
		
		<view class="line top40"></view> -->
		<view class="" @click="pay" v-if="!show">
		...
		<view class="mask flex-column" v-if="maskShow">
			...
			<view class="relative mask-box center " v-if="maskShow==2">
				...
				<view class="flex between bottom40">
					<view class="" v-for="(item, index) in icons" :key="index">
...
<script>
	...
	export default {
		...
		methods: {
			/**
			 * 判断网页是否是 app webview访问
			 * 
			 * 参考：
			 * https://blog.csdn.net/m0_46365130/article/details/120869204
			 * https://icode.best/i/05047141485560
			 */
			_browserIsAPP() {
        const sUserAgent = navigator.userAgent
				// const u = navigator.userAgent,
				// const isiOS = !!sUserAgent.match(/\(i[^;]+;( U;)? CPU.+Mac OS X/);
				const isiOS = sUserAgent.match(/(iPhone\sOS)\s([\d_]+)/);
				const isAndroid = sUserAgent.indexOf('Android') > -1 || sUserAgent.indexOf('Adr') > -1; // android 终端

				// console.log(isiOS)
				// console.log(isAndroid)
				// console.log(sUserAgent.match(/Web[kK]it[\/]{0,1}([\d.]+)/))
