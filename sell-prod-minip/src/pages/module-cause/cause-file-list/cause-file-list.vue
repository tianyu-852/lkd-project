<template>
  <view class="main">
    <u-navbar class="u-navbar" :is-back="false" :border-bottom="false" :background="{ backgroundColor: '#fff' }" title="文件列表" title-color="#030303">
...
export default {
  ...
  onLoad() {
    // 上次已选中的文件
    this.prevedCloudFileList = this.$Route.query.prevedCloudFileList;
  ...
  methods: {
    ...
    async _getCloudStorageFileList(page = 1) {
      ...
      try {
        const { data: cloudFileData } = await this.$api.getCloudStorageFileList(param);
        // 添加是否选中状态
    ...
    async chooseFile() {
      ...
      for (const fItem of tempFiles) {
        ...
        const { data } = await this.$api.postCloudStorageFile(param);
        console.log(data);
      }
    ...
    async onDelFileModalConfirmEvt() {
      ...
      try {
        const data = await this.$api.delCloudStorageFile(param);
