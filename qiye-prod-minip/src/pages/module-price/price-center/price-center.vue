  ...
  <view class="main">
    ...
    <view class="company-vip-module">
      <view class="header-wrap">
        ...
        <view style="color: #0085FF;" @click="$commonUtil.routeNavTo('module-price/price-diff/price-diff', { diffImg: companyDiffImg })">对比图示</view>
      </view>
    ...
    <u-popup v-model="isShowShareModePopup" mode="bottom" border-radius="24" safe-area-inset-bottom :mask-custom-style="{ background: 'rgba(0, 0, 0, .2)' }" @close="onShareModePopupCloseEvt">
      <view class="upopup-share-box" @click.stop="$commonUtil.stopPropagation">
        ...
        <label class="share-label" for="shareBtnId">
          <view class="type-item" @click="isShareDiscountPirce = true">分享卡片</view>
        </label>
      </view>
    </u-popup>
...
export default {
  ...
  data() {
    return {
      paletteTmp: paletteTmp, // 海报 palette 模板
      ...
      shareModePopupList: [{ name: '对公打款' }, { name: '二维码支付(微信)' }, { name: '二维码支付(支付宝)' }], // 分享方法 弹框
  ...
  async onShareAppMessage() {
    ...
    if (this.shareVipType === 2) {
      ...
      shareVipType = 2; // 企业类型 vip
      shareVipId = companyVipId;
    // 分享个人 vip
      ...
      shareVipType = 1; // 个人类型 vip
      shareVipId = personVipId;
    }
    ...
    return {
      ...
      path: `/pages/module-common/vip-pay/vip-pay?sellerFlag=${ sellerFlag }&shareVipType=${ shareVipType }&shareVipId=${ shareVipId }`,
