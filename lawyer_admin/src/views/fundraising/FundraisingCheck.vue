        ...
        <a-card>
          <a-row>
            ...
            <a-col :span="5" :offset="5">
              ...
              />
              <!-- :default-value="[moment(timeDate.startDate, dateFormat), moment(timeDate.endDate, dateFormat)]" -->
            </a-col>
          </a-row>
        </a-card>
      </a-col>
          ...
          >
            ...
            <a-table-column key="user" title="申请用户">
              <template slot-scope="record">
                <user-vip-cell
                  ...
                  @open="openUser(record.create.user_id, 'user')"
                />
              </template>
            </a-table-column>
            ...
            <a-table-column key="action" title="操作">
              <template slot-scope="text, record">
                <a-button type="primary" style="margin: 5px" @click="goDetails(record)"> 审核 </a-button>
                <a-popconfirm
                  title="请确认是否要删除该案件"
                  ok-text="确认"
                  cancel-text="取消"
                  @confirm="confirm(record.fundraising_registration_id)"
                  @cancel="this.$message.error('取消删除')"
                >
                  <a-button type="danger" style="margin: 5px">删除</a-button>
                </a-popconfirm>
                <a-button
                  disabled
                  v-if="true"
  ...
  methods: {
    getFundraisingPage(page, status, search, startDate, endDate) {
      getFundraisingPage(page, status, search, startDate, endDate).then((data) => {
        ...
        if (this.status || this.search || this.timeDate.startDate || this.timeDate.endDate) {
        } else {
