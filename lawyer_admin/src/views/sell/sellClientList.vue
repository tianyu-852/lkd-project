<template>
  <a-layout class="main">
    <a-layout-header class="top">
      <div v-if="layoutType === 1" class="back">
        <span class="icon-back" @click="$router.back(-1)">←</span>
      </div>
      
      <span class="focus-text">客户人数：{{ pagination.total }}</span>
      ...
      <template v-if="userInfo.seller && !userInfo.seller.is_customer">
        <!-- 导出vip select -->
    ...
    <a-card class="content">
      ...
      <div class="tool-wrap">
        <template v-if="selectedTableRowKeys.length">
          ...
          <a-button icon="export" size="small" style="margin: 0 10px;" @click="openTransferModal">转移</a-button>
        </template>
        <a-button v-if="selectedUserIdList.length" icon="to-top" size="small" style="margin: 0 10px;" @click="openVipModal(null, 2)">会员服务</a-button>
        ...
        <!-- 会员类型 -->
        <!-- <div v-if="layoutType === 1" style="margin-left: auto;">
          <span style="font-size: 16px; font-weight: 500;">会员类型：</span>
          <a-select style="width: 100px; margin-top: 5px;" v-model="vip_type" @change="getSearchData">
            <a-select-option v-for="vItem in vipTypeSelectList" :value="vItem.value" :key="vItem.value">
              {{ vItem.label }}
            </a-select-option>
          </a-select>
        </div> -->

        <a-button size="small" style="margin-left: auto;" @click="navToSellClientVipList(1)">已开通会员</a-button>
        <a-button size="small" style="margin-left: 10px;" @click="navToSellClientVipList(3)">已过期会员</a-button>

        <a-button icon="sync" size="small" style="margin-left: 10px;" @click="_getQiyeClientList(pagination.current)">刷新</a-button>
      </div>
      ...
      >
        <!-- id -->
        <!-- <a-table-column dataIndex="customer_log_id" title="ID" align="center" fixed="left" width="80px" /> -->

        <!-- 姓名 -->
        <!-- <a-table-column dataIndex="name" title="昵称" align="center" fixed="left" width="150px" /> -->
        ...
        <a-table-column key="user" title="手机号码" align="center" fixed="left" width="160px">
          <template slot-scope="record">
            <div class="column-user-phone-wrap">
              ...
              <!-- <div v-if="handleShowCallBtn(record.created_at)" class="call-wrap" @click="workbenchCallPhone(record)">
                <img class="call-img" :src="require('@/assets/service/call-phone.png')" />
              </div> -->
              <div v-if="userInfo.name === '吴薇' || userInfo.name === '吴文君'" class="call-wrap" @click="workbenchCallPhone(record)">
                <img class="call-img" :src="require('@/assets/service/call-phone.png')" />
              </div>
            </div>
          </template>
        ...
        <!-- 拨打状态 -->
        <!-- <a-table-column dataIndex="is_ali_mq" title="拨打状态" align="center" width="150px" /> -->

        <!-- 手机归属地 -->
        <!-- <a-table-column dataIndex="mobile_location" title="手机归属地" align="center" /> -->

        <!-- 位置 -->
        <!-- <a-table-column dataIndex="location" title="位置" align="center" /> -->

        <!-- 注册时间 -->
        <a-table-column key="created_at" dataIndex="created_at" title="注册时间" align="center" width="150px" />

        <!-- 修改时间 -->
        <!-- <a-table-column key="updated_at" dataIndex="updated_at" title="修改时间" align="center" /> -->
        ...
        <!-- 开通时间 -->
        <a-table-column v-if="vip_type === 1" dataIndex="vip_new_time" title="开通时间" align="center" width="150px" />

        <!-- 过期时间 -->
        <a-table-column v-if="vip_type === 1" dataIndex="vip_expired_time" title="过期时间" align="center" width="150px" />
        
        <!-- 来源信息 -->
        <!-- <a-table-column align="center" width="134px">
          <div slot="title" style="display: flex; flex-direction: column; justify-content: center; align-items: center">
            <span>来源信息</span>
            <a-select style="width: 100px; margin-top: 5px;" v-model="source_tag" @change="getSearchData">
        ...
        <a-table-column key="action" title="操作" align="center" fixed="right" width="190px">
          <template slot-scope="record">
            ...
            <a-button type="link" style="padding: 0;" @click="openVipModal(record, 1)">会员服务</a-button>
          </template>
        </a-table-column>
      </a-table>
    </a-card>
    ...
    >
      <div class="vip-modal">
        <div class="modal-header">{{ vipModalType === 2 ? '批量发送会员服务' : '发送会员服务' }}</div>
        <div class="modal-body">
          <a-radio-group v-model="vipModal.checkedValue">
            <a-radio class="textarea-item" :value="1">
              <a-textarea v-model="vipModal.content_one" class="input-textarea" placeholder="请输入" :auto-size="{ minRows: 4 }" />
            </a-radio>
            <a-radio class="textarea-item" :value="2">
              <a-textarea v-model="vipModal.content_two" class="input-textarea" placeholder="请输入" :auto-size="{ minRows: 4 }" />
            </a-radio>
          </a-radio-group>
        </div>

        <!-- 单个发送会员服务 -->
        <div v-if="vipModalType === 1" class="modal-footer">
        ...
        <!-- 批量发送会员服务 -->
        <div v-if="vipModalType === 2" class="modal-footer">
          <a-button v-for="(vItem, vIndex) in vipList" :key="vIndex" type="primary" @click="sendBatchVipMsgToUser(vItem.vip_id)">{{ vItem.vip_name }}{{ vItem.origin_price }}元</a-button>
        </div>
      </div>
    </a-modal>
    ...
    <!-- 阿里云呼 -->
    <div v-if="userInfo.name === '吴薇' || userInfo.name === '吴文君'" class="workbench-module">
...
const VIP_SESSION_KEY = 'sell_vip_session' // localStorage 缓存 key
const VIP_SESSION_CONTENT = '您好，我是刚刚跟您电话沟通的法律管家，您可以直接点击下方开通咱们的会员，享受优质法律咨询'
// vip modal info
const VIP_MODAL_INFO = {
  isShowVipModal: false, // 是否显示
  userId: '', // 用户 id
  checkedValue: 1, // 选择的内容
  content_one: VIP_SESSION_CONTENT,
  content_two: VIP_SESSION_CONTENT,
}
...
export default {
  ...
  data() {
    return {
      ...
      vipList: [], // vip列表
      vipModalType: 1, // vip会话弹框类型 1: 单个发送 2: 批量发送
      selectedUserIdList: [], // 批量选中的 userId
  ...
  mounted() {
    this.layoutType = this.$route.query.layoutType // url 布局类型
  ...
  methods: {
     ...
     * 判断是否显示拨号按钮
     */
    handleShowCallBtn(createdAt) {
      const isBefore = moment(createdAt).isAfter('2022-6-14 12:00')
      return isBefore
    },
     ...
     async _getSellServiceList() {
       ...
       try {
         ...
         if (this.$route.query.sellUserId) {
           ...
           // 导出销售过滤列表
           this.exportSellSelectList = this.sellSelectList
           this.exportSellId = this.customer_id
         }
       } catch (error) {
         console.log(error)
       }
    ...
    onTableRowSelectChange(selectedRowKeys, selectedRows) {
      ...
      // 选中的用户 id
      this.selectedUserIdList = selectedRows.map(item => item.user_id)
    },
     ...
     * 打开会员服务 modal
     */
    openVipModal(record, vipModalType) {
      this.vipModalType = vipModalType
      if (this.vipModalType === 1) {
        this.vipModal.userId = record.user_id
      }

      this.vipModal.isShowVipModal = true
      this.vipModal.content_one = (window.localStorage.getItem(VIP_SESSION_KEY) && JSON.parse(window.localStorage.getItem(VIP_SESSION_KEY))[0]) ? JSON.parse(window.localStorage.getItem(VIP_SESSION_KEY))[0] : VIP_SESSION_CONTENT
      this.vipModal.content_two = (window.localStorage.getItem(VIP_SESSION_KEY) && JSON.parse(window.localStorage.getItem(VIP_SESSION_KEY))[1]) ? JSON.parse(window.localStorage.getItem(VIP_SESSION_KEY))[1] : VIP_SESSION_CONTENT
    },
    ...
    async sendVipMsgToUser(vipId) {
      const content = this.vipModal.checkedValue === 1 ? this.vipModal.content_one : this.vipModal.content_two
      if (!content.trim()) return this.$message.warning('请输入内容')
      const param = { vip_id: vipId, invitees_user_id: this.vipModal.userId, content }

      // localStorage 会话消息
      window.localStorage.setItem(VIP_SESSION_KEY, JSON.stringify([ this.vipModal.content_one, this.vipModal.content_two ]))
     ...
     * 发送批量会员服务消息
     */
    async sendBatchVipMsgToUser(vipId) {
      // 消息内容
      const content = this.vipModal.checkedValue === 1 ? this.vipModal.content_one : this.vipModal.content_two
      if (!content.trim()) return this.$message.warning('请输入内容')
      // localStorage 会话消息
      ...
      try {
        for (const userId of this.selectedUserIdList) {
          console.log(userId)
          const param = { vip_id: vipId, invitees_user_id: userId, content }
          const { data } = await sellApi.sendVipMsgApi(param)
        }
     ...
     * 进入销售客户会员开通页面
     */
    navToSellClientVipList(vipType) {
      const sellUserId = String(this.customer_id)
      this.$router.push({ name: 'sellClientVipList', query: { sellUserId, vipType} })
    },
  }
}
</script>
