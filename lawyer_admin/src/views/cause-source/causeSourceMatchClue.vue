  ...
  <a-layout class="main">
    <a-layout-header class="top">
      <div class="back">
        ...
        <template v-if="lawyerCaseSourceDetail.user">
          <span>{{ lawyerCaseSourceDetail.user.nickname }}</span>
          <span style="margin-left: 30px;">今日匹配数量：{{ lawyerCaseSourceDetail.today_match_num }}/{{ lawyerCaseSourceDetail.day_num }}</span>
          <span style="margin-left: 30px;">匹配总数量：{{ lawyerCaseSourceDetail.already_matched_num }}/{{ lawyerCaseSourceDetail.total_num }}</span>
          <span style="margin-left: 30px;">案源地区：{{ lawyerCaseSourceDetail.province }}/{{ lawyerCaseSourceDetail.city }}</span>
        </template>
      </div>
    ...
    <a-layout-header class="tool-module">
      <span style="font-size: 16px; font-weight: 600; margin-right: 20px;">线索数量：{{ pagination.total }}</span>
      ...
      <span style="margin-left: 20px; margin-right: 10px;">24小时内: </span>
      <a-switch v-model="time24HourFactor" @change="onTime24HourSwitchChangeEvt" />
      
      <!-- <span style="font-size: 16px; font-weight: 600;">标签筛选：</span>
        ...
        <div class="table-clue">
          <div v-for="(tItem, tIndex) in tableDataList" :key="tIndex" class="clue-table-item">
            ...
            <div class="table-column-wrap">
              ...
              <div class="column-cell">
                <div v-if="tIndex === 0" class="column-head"><span>是否已匹配给该律师</span></div>
                <div class="column-body">
                  <span v-if="tItem.is_match" style="color: #41b583;">已匹配</span>
                  <span v-else>未匹配</span>
                </div>
              </div>

              <div class="column-cell">
                <div v-if="tIndex === 0" class="column-head"><span>是否已联系过该律师</span></div>
                <div class="column-body">
                  <span v-if="tItem.is_call" style="color: #41b583;">已联系过</span>
                  <span v-else>未联系过</span>
                </div>
              </div>
              ...
              <div class="column-cell">
                ...
                <div class="column-body">
                  <a-button v-if="tItem.is_match" disabled type="primary">单条匹配 ({{ tItem.use_num }}/{{ tItem.upper }})</a-button>
                  <a-popconfirm v-else-if="!tItem.upper" :title="'是否这条线索匹配给「' + lawyerCaseSourceDetail.user.nickname + '」?'" @confirm="confrimSingleMatchToLawyer(tItem)">
                    <a-button type="primary">单条匹配</a-button>
                  </a-popconfirm>
                  <a-popconfirm v-else-if="tItem.use_num < tItem.upper" @confirm="confrimSingleMatchToLawyer(tItem)">
                    <p slot="icon"></p>
                    <template slot="title">
                      <div style="margin-bottom: 4px;">是否这条线索匹配给「'{{ lawyerCaseSourceDetail.user.nickname }}'」?</div>
                      <a-input v-model="singleMatchRemark" placeholder="备注（可选）" />
                    </template>
                    <a-button type="primary">单条匹配 ({{ tItem.use_num }}/{{ tItem.upper }})</a-button>
                  </a-popconfirm>
...
export default {
  ...
  data() {
    return {
      ...
      lawyerCaseSourceDetail: {}, // 律师案源vip详情
      ...
      // 时间条件
      timeFactorInfo: {
        // startDate: moment().add(-15, 'days').startOf('day').format('YYYY-MM-DD HH:mm:ss'), // 开始时间
  ...
  async created() {
    ...
    await this._getLawyerCaseSourceVipDetail()
    this._getCauseSourceClueData()

    // console.log(moment().add(-24, 'hours').format('YYYY-MM-DD HH:mm:ss'))
  ...
  methods: {
     ...
     * 获取案源vip 详情
     */
    async _getLawyerCaseSourceVipDetail() {
      const param = { user_id: this.lawyerUserId, }
      try {
        const { data: caseSourceVipDetailData } = await causeSourceApi.getLawyerCaseSourceVipDetailApi(param)
        this.lawyerCaseSourceDetail = caseSourceVipDetailData
      } catch (error) {
        console.log(error)
      }
    },
    ...
    async _getCauseSourceClueData() {
      ...
      const param = {
        ...
        city_id: this.lawyerCaseSourceDetail.city_id,
     ...
     * 确定单个线索匹配给律师
     */
    async confrimSingleMatchToLawyer(record) {
      const param = { lawyer_user_id: this.lawyerUserId, user_id: record.user.user_id, matching_remark: this.singleMatchRemark.trim()  }

      try {
        await causeSourceApi.postCaseSourceVipMatchClueApi(param)
        this.singleMatchRemark = ''
