<template>
  <a-layout class="main">
    <a-layout-header class="top">
      <span class="focus-text">可疑空号律师数量：{{ pagination.total }}</span>
      <a-input
        allow-clear
...
<script>
import { deleteLawyer, getLawyerEmptyNumber, getLawyerBetter } from '@/api/lawyer'
...
export default {
  ...
  created() {
    const condition = JSON.parse(sessionStorage.getItem('lawyerEmptyCondition'))
    if (condition) {
      this.pagination.current = condition.current
      this.search = condition.search
      // window.sessionStorage.removeItem('lawyerEmptyCondition')
  ...
  methods: {
    beforeunloadFn(e) {
      sessionStorage.setItem('lawyerEmptyCondition', JSON.stringify(this.conditionData))
    },
