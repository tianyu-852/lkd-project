  ...
  <a-layout class="main">
    <a-layout-header class="top">
      <div class="back">
        ...
        <span>优选「体验期」律师：{{ pagination.total }}</span>
      </div>
      ...
      <template v-if="curStatusValFactor == 1">
        <span style="margin-left: 20px; margin-right: 4px;">后续续费状态：</span>
        <a-select style="width: 140px;" v-model="nextStatusValFactor" @change="_getBestExperienceList()">
          <a-select-option v-for="sItem in nextSelectList" :value="sItem.value" :key="sItem.value">
            {{ sItem.label }}
          </a-select-option>
        </a-select>
      </template>
        ...
        <a-table-column key="nickname" title="律师" align="center" :width="260">
          <template slot-scope="record">
            <div style=" display: flex; flex-direction: column; align-items: center; justify-content: center; align-content: center; flex-wrap: nowrap; ">
              ...
              <a-tag v-if="record.is_have_experience" color="#F56C6C" style="margin-top: 4px; border-radius: 4px;">体验期</a-tag>
              <br />
...
export default {
  ...
  data() {
    return {
      ...
      nextSelectList: [{ label: '未续费', value: 0 }, { label: '已续费', value: 1 }], // 续费状态 select 列表
      nextStatusValFactor: 0, // 续费状态
    }
  },
  created() {
  ...
  methods: {
     ...
     */
     async _getBestExperienceList(page = 1) {
       ...
       // 是否续费
       if (this.curStatusValFactor) param.many = this.nextStatusValFactor
