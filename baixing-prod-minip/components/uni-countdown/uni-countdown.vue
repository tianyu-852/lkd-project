<template>
	<view class="uni-countdown flex hcenter" :style="{color: color}">
		...
		<text :style="{ color: color }" class="uni-countdown__splitor">{{ showColon ? ':' : '分' }}</text>
		<text :style="{ borderColor: borderColor, color: color, backgroundColor: backgroundColor }"
	...
	export default {
		...
		methods: {
			...
			countDown() {
				...
				if (seconds <= 0) {
					this.timeUp()
					seconds = this.toSeconds(this.timestamp, this.day, this.hour, this.minute, this.second)
				}
