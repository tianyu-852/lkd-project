<template>
	<view class="">
		  <movable-area class="drag-sort" :style="{height:boxHeight }" id="drag">
			...
			:style="styleObject"
			:class="{'active': active == index, 'vh-1px-t': item.index > 0,'enable':index==0&& isEdit && !onlyClose}">
			  <view class="item">
				  <text>{{item[props.label]}}</text>
				  <image v-if="isEdit && (index>0|| onlyClose)" :src="onlyClose?'/static/images/tj_icon.png':'/static/images/gb_icon.svg'" class="closeImg" mode=""></image>
			  </view>
			</movable-view>
		  </movable-area>
	</view>
...
export default {
  ...
  methods: {
    ...
    touchstart (e) {
		if(this.onlyClose)return
		if(Number(e.currentTarget.dataset.index)==0 || !this.isEdit)return
      // 计算 x y 轴点击位置
